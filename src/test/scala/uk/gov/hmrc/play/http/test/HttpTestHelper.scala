...
trait HttpTestHelper extends ResponseMatchers {
  ...
  def verifyGETStatusCodeOnly(url: String, expectedStatus: Int, headers: Option[HeaderCarrier] = None)(
    implicit hc: HeaderCarrier                                                                 = headers.getOrElse(HeaderCarrier())): Unit =
    buildRequest(url).get() should have(status(expectedStatus))

  def verifyPOSTStatusCodeOnly(
    url: String,
    expectedStatus: Int,
    body: JsValue                  = JsNull,
    headers: Option[HeaderCarrier] = None)(implicit hc: HeaderCarrier = headers.getOrElse(HeaderCarrier())): Unit =
